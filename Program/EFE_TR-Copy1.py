...
def ev_KA(f0, r, dR, N, OPL):
    ...
    p1 = d_r(Dal, dR) + ghost_derivative(DB, dR)[2:]
    p2 = Dal**2 + 0.5 * (- Dal * DA[2:] + DB[2:]**2 - DA[2:] * DB[2:])
    p3 = - psi[2:]**4 * A[2:] * KA[2:] * (KA[2:] + 2*KB[2:])
    p4 = - (DA[2:] - 2 * DB[2:]) / r[2:]
    p5 = 4 * d2_r(np.log(psi[2:]), dR) + d_r(np.log(psi[2:]), dR) * (2 * DB[2:] - 2 * DA[2:] - 2 * Dal + 4 / r[2:])
...
def ev_KB(f0, r, dR, N, OPL):
    ...
    p5 = 4 * d2_r(np.log(psi[2:]), dR) + d_r(np.log(psi[2:]), dR) * (8*d_r(np.log(psi[2:]), dR) + 4*Dal + 6*DB[2:] -2*DA[2:] + 12 / r[2:])
